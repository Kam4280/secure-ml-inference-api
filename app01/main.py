...
def load_secure_model(model_path, sig_path, pub_key_path):
    # Perform signature verification using the public key
    result = subprocess.run(
        ["cosign", "verify-blob", "--key", pub_key_path, "--signature", sig_path, model_path],
        capture_output=True, text=True
    )
    
    if result.returncode != 0:
        # CRITICAL SECURITY ALERT: Do NOT load the model!
        print("ALERT: Model tampering detected! Integrity verification failed.")
        raise PermissionError("Security Violation: Untrusted model weights.")
    
    print("Integrity Verified: Loading model into memory...")
    # Now it is safe to proceed with loading (e.g., torch.load, pickle.load)   